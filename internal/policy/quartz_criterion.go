...
func NewQuartzCriterion() QuartzCriterion {
	return QuartzCriterion{
		Mineral:     "quartz",
		Channel:     "isotope-05",
		Lower:       45.0,
		Upper:       115.0,
...
func (c QuartzCriterion) Validate() error {
...
func (c QuartzCriterion) Assess(o QuartzObservation) (QuartzAssessment, error) {
	if err := c.Validate(); err != nil {
		return QuartzAssessment{}, err
	}
	if o.Channel != c.Channel {
		return QuartzAssessment{}, fmt.Errorf("channel mismatch")
	}
	...
	if math.IsNaN(o.Value) || math.IsInf(o.Value, 0) {
		return QuartzAssessment{}, fmt.Errorf("non-finite reading")
	}
	if o.Value < c.Lower {
	...
	return QuartzAssessment{Mineral: c.Mineral, Accepted: accepted, Score: score, Notes: notes}, nil
}

func (c QuartzCriterion) Explain(o QuartzObservation) string {
	a, err := c.Assess(o)
	if err != nil {
		return err.Error()
	}
