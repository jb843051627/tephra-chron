...
type TitaniteObservation struct {
	Channel    string
	Value      float64
	Drift      float64
	Runs       int
...
type TitaniteAssessment struct {
	Mineral  string
	Accepted bool
	Score    float64
	Notes    []string
...
func NewTitaniteCriterion() TitaniteCriterion {
	return TitaniteCriterion{
		Mineral:     "titanite",
		Channel:     "isotope-21",
		Lower:       61.0,
		Upper:       131.0,
...
func (c TitaniteCriterion) Validate() error {
...
func (c TitaniteCriterion) Assess(o TitaniteObservation) (TitaniteAssessment, error) {
	if err := c.Validate(); err != nil {
		return TitaniteAssessment{}, err
	}
	if o.Channel != c.Channel {
		return TitaniteAssessment{}, fmt.Errorf("channel mismatch")
	}
	...
	if math.IsNaN(o.Value) || math.IsInf(o.Value, 0) {
		return TitaniteAssessment{}, fmt.Errorf("non-finite reading")
	}
	if o.Value < c.Lower {
	...
	return TitaniteAssessment{Mineral: c.Mineral, Accepted: accepted, Score: score, Notes: notes}, nil
}

func (c TitaniteCriterion) Explain(o TitaniteObservation) string {
	a, err := c.Assess(o)
	if err != nil {
		return err.Error()
	}
