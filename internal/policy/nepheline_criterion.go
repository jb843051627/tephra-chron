...
type NephelineCriterion struct {
	Mineral     string
	Channel     string
	Lower       float64
	Upper       float64
...
type NephelineObservation struct {
	Channel    string
	Value      float64
	Drift      float64
	Runs       int
...
type NephelineAssessment struct {
	Mineral  string
	Accepted bool
	Score    float64
	Notes    []string
...
func NewNephelineCriterion() NephelineCriterion {
	return NephelineCriterion{
		Mineral:     "nepheline",
		Channel:     "isotope-23",
		Lower:       63.0,
		Upper:       133.0,
...
func (c NephelineCriterion) Validate() error {
...
func (c NephelineCriterion) Assess(o NephelineObservation) (NephelineAssessment, error) {
	if err := c.Validate(); err != nil {
		return NephelineAssessment{}, err
	}
	if o.Channel != c.Channel {
		return NephelineAssessment{}, fmt.Errorf("channel mismatch")
	}
	...
	if math.IsNaN(o.Value) || math.IsInf(o.Value, 0) {
		return NephelineAssessment{}, fmt.Errorf("non-finite reading")
	}
	if o.Value < c.Lower {
	...
	return NephelineAssessment{Mineral: c.Mineral, Accepted: accepted, Score: score, Notes: notes}, nil
}

func (c NephelineCriterion) Explain(o NephelineObservation) string {
	a, err := c.Assess(o)
	if err != nil {
		return err.Error()
	}
