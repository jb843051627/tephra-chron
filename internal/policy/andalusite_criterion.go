...
type AndalusiteCriterion struct {
	Mineral     string
	Channel     string
	Lower       float64
	Upper       float64
...
type AndalusiteObservation struct {
	Channel    string
	Value      float64
	Drift      float64
	Runs       int
...
type AndalusiteAssessment struct {
	Mineral  string
	Accepted bool
	Score    float64
	Notes    []string
...
func NewAndalusiteCriterion() AndalusiteCriterion {
	return AndalusiteCriterion{
		Mineral:     "andalusite",
		Channel:     "isotope-25",
		Lower:       65.0,
		Upper:       135.0,
...
func (c AndalusiteCriterion) Validate() error {
...
func (c AndalusiteCriterion) Assess(o AndalusiteObservation) (AndalusiteAssessment, error) {
	if err := c.Validate(); err != nil {
		return AndalusiteAssessment{}, err
	}
	if o.Channel != c.Channel {
		return AndalusiteAssessment{}, fmt.Errorf("channel mismatch")
	}
	...
	if math.IsNaN(o.Value) || math.IsInf(o.Value, 0) {
		return AndalusiteAssessment{}, fmt.Errorf("non-finite reading")
	}
	if o.Value < c.Lower {
	...
	return AndalusiteAssessment{Mineral: c.Mineral, Accepted: accepted, Score: score, Notes: notes}, nil
}

func (c AndalusiteCriterion) Explain(o AndalusiteObservation) string {
	a, err := c.Assess(o)
	if err != nil {
		return err.Error()
	}
