...
type SillimaniteCriterion struct {
	Mineral     string
	Channel     string
	Lower       float64
	Upper       float64
...
type SillimaniteObservation struct {
	Channel    string
	Value      float64
	Drift      float64
	Runs       int
...
type SillimaniteAssessment struct {
	Mineral  string
	Accepted bool
	Score    float64
	Notes    []string
...
func NewSillimaniteCriterion() SillimaniteCriterion {
	return SillimaniteCriterion{
		Mineral:     "sillimanite",
		Channel:     "isotope-27",
		Lower:       67.0,
		Upper:       137.0,
...
func (c SillimaniteCriterion) Validate() error {
...
func (c SillimaniteCriterion) Assess(o SillimaniteObservation) (SillimaniteAssessment, error) {
	if err := c.Validate(); err != nil {
		return SillimaniteAssessment{}, err
	}
	if o.Channel != c.Channel {
		return SillimaniteAssessment{}, fmt.Errorf("channel mismatch")
	}
	...
	if math.IsNaN(o.Value) || math.IsInf(o.Value, 0) {
		return SillimaniteAssessment{}, fmt.Errorf("non-finite reading")
	}
	if o.Value < c.Lower {
	...
	return SillimaniteAssessment{Mineral: c.Mineral, Accepted: accepted, Score: score, Notes: notes}, nil
}

func (c SillimaniteCriterion) Explain(o SillimaniteObservation) string {
	a, err := c.Assess(o)
	if err != nil {
		return err.Error()
	}
