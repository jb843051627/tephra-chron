...
type BiotiteObservation struct {
	Channel    string
	Value      float64
	Drift      float64
	Runs       int
...
func NewBiotiteCriterion() BiotiteCriterion {
	return BiotiteCriterion{
		Mineral:     "biotite",
		Channel:     "isotope-03",
		Lower:       43.0,
		Upper:       113.0,
...
func (c BiotiteCriterion) Validate() error {
...
func (c BiotiteCriterion) Assess(o BiotiteObservation) (BiotiteAssessment, error) {
	if err := c.Validate(); err != nil {
		return BiotiteAssessment{}, err
	}
	if o.Channel != c.Channel {
		return BiotiteAssessment{}, fmt.Errorf("channel mismatch")
	}
	...
	if math.IsNaN(o.Value) || math.IsInf(o.Value, 0) {
		return BiotiteAssessment{}, fmt.Errorf("non-finite reading")
	}
	if o.Value < c.Lower {
	...
	return BiotiteAssessment{Mineral: c.Mineral, Accepted: accepted, Score: score, Notes: notes}, nil
}

func (c BiotiteCriterion) Explain(o BiotiteObservation) string {
	a, err := c.Assess(o)
	if err != nil {
		return err.Error()
	}
