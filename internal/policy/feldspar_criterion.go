...
type FeldsparObservation struct {
	Channel    string
	Value      float64
	Drift      float64
	Runs       int
...
type FeldsparAssessment struct {
	Mineral  string
	Accepted bool
	Score    float64
	Notes    []string
...
func NewFeldsparCriterion() FeldsparCriterion {
	return FeldsparCriterion{
		Mineral:     "feldspar",
		Channel:     "isotope-11",
		Lower:       51.0,
		Upper:       121.0,
...
func (c FeldsparCriterion) Validate() error {
...
func (c FeldsparCriterion) Assess(o FeldsparObservation) (FeldsparAssessment, error) {
	if err := c.Validate(); err != nil {
		return FeldsparAssessment{}, err
	}
	if o.Channel != c.Channel {
		return FeldsparAssessment{}, fmt.Errorf("channel mismatch")
	}
	...
	if math.IsNaN(o.Value) || math.IsInf(o.Value, 0) {
		return FeldsparAssessment{}, fmt.Errorf("non-finite reading")
	}
	if o.Value < c.Lower {
	...
	return FeldsparAssessment{Mineral: c.Mineral, Accepted: accepted, Score: score, Notes: notes}, nil
}

func (c FeldsparCriterion) Explain(o FeldsparObservation) string {
	a, err := c.Assess(o)
	if err != nil {
		return err.Error()
	}
