...
type KyaniteObservation struct {
	Channel    string
	Value      float64
	Drift      float64
	Runs       int
...
func NewKyaniteCriterion() KyaniteCriterion {
	return KyaniteCriterion{
		Mineral:     "kyanite",
		Channel:     "isotope-26",
		Lower:       66.0,
		Upper:       136.0,
...
func (c KyaniteCriterion) Validate() error {
...
func (c KyaniteCriterion) Assess(o KyaniteObservation) (KyaniteAssessment, error) {
	if err := c.Validate(); err != nil {
		return KyaniteAssessment{}, err
	}
	if o.Channel != c.Channel {
		return KyaniteAssessment{}, fmt.Errorf("channel mismatch")
	}
	...
	if math.IsNaN(o.Value) || math.IsInf(o.Value, 0) {
		return KyaniteAssessment{}, fmt.Errorf("non-finite reading")
	}
	if o.Value < c.Lower {
	...
	return KyaniteAssessment{Mineral: c.Mineral, Accepted: accepted, Score: score, Notes: notes}, nil
}

func (c KyaniteCriterion) Explain(o KyaniteObservation) string {
	a, err := c.Assess(o)
	if err != nil {
		return err.Error()
	}
