...
type AllaniteObservation struct {
	Channel    string
	Value      float64
	Drift      float64
	Runs       int
...
type AllaniteAssessment struct {
	Mineral  string
	Accepted bool
	Score    float64
	Notes    []string
...
func NewAllaniteCriterion() AllaniteCriterion {
	return AllaniteCriterion{
		Mineral:     "allanite",
		Channel:     "isotope-19",
		Lower:       59.0,
		Upper:       129.0,
...
func (c AllaniteCriterion) Validate() error {
...
func (c AllaniteCriterion) Assess(o AllaniteObservation) (AllaniteAssessment, error) {
	if err := c.Validate(); err != nil {
		return AllaniteAssessment{}, err
	}
	if o.Channel != c.Channel {
		return AllaniteAssessment{}, fmt.Errorf("channel mismatch")
	}
	...
	if math.IsNaN(o.Value) || math.IsInf(o.Value, 0) {
		return AllaniteAssessment{}, fmt.Errorf("non-finite reading")
	}
	if o.Value < c.Lower {
	...
	return AllaniteAssessment{Mineral: c.Mineral, Accepted: accepted, Score: score, Notes: notes}, nil
}

func (c AllaniteCriterion) Explain(o AllaniteObservation) string {
	a, err := c.Assess(o)
	if err != nil {
		return err.Error()
	}
