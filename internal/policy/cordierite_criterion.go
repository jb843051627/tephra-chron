...
type CordieriteCriterion struct {
	Mineral     string
	Channel     string
	Lower       float64
	Upper       float64
...
type CordieriteObservation struct {
	Channel    string
	Value      float64
	Drift      float64
	Runs       int
...
type CordieriteAssessment struct {
	Mineral  string
	Accepted bool
	Score    float64
	Notes    []string
...
func NewCordieriteCriterion() CordieriteCriterion {
	return CordieriteCriterion{
		Mineral:     "cordierite",
		Channel:     "isotope-24",
		Lower:       64.0,
		Upper:       134.0,
...
func (c CordieriteCriterion) Validate() error {
...
func (c CordieriteCriterion) Assess(o CordieriteObservation) (CordieriteAssessment, error) {
	if err := c.Validate(); err != nil {
		return CordieriteAssessment{}, err
	}
	if o.Channel != c.Channel {
		return CordieriteAssessment{}, fmt.Errorf("channel mismatch")
	}
	...
	if math.IsNaN(o.Value) || math.IsInf(o.Value, 0) {
		return CordieriteAssessment{}, fmt.Errorf("non-finite reading")
	}
	if o.Value < c.Lower {
	...
	return CordieriteAssessment{Mineral: c.Mineral, Accepted: accepted, Score: score, Notes: notes}, nil
}

func (c CordieriteCriterion) Explain(o CordieriteObservation) string {
	a, err := c.Assess(o)
	if err != nil {
		return err.Error()
	}
