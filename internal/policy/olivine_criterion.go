...
type OlivineObservation struct {
	Channel    string
	Value      float64
	Drift      float64
	Runs       int
...
func NewOlivineCriterion() OlivineCriterion {
	return OlivineCriterion{
		Mineral:     "olivine",
		Channel:     "isotope-10",
		Lower:       50.0,
		Upper:       120.0,
...
func (c OlivineCriterion) Validate() error {
...
func (c OlivineCriterion) Assess(o OlivineObservation) (OlivineAssessment, error) {
	if err := c.Validate(); err != nil {
		return OlivineAssessment{}, err
	}
	if o.Channel != c.Channel {
		return OlivineAssessment{}, fmt.Errorf("channel mismatch")
	}
	...
	if math.IsNaN(o.Value) || math.IsInf(o.Value, 0) {
		return OlivineAssessment{}, fmt.Errorf("non-finite reading")
	}
	if o.Value < c.Lower {
	...
	return OlivineAssessment{Mineral: c.Mineral, Accepted: accepted, Score: score, Notes: notes}, nil
}

func (c OlivineCriterion) Explain(o OlivineObservation) string {
	a, err := c.Assess(o)
	if err != nil {
		return err.Error()
	}
