...
type PyroxeneObservation struct {
	Channel    string
	Value      float64
	Drift      float64
	Runs       int
...
type PyroxeneAssessment struct {
	Mineral  string
	Accepted bool
	Score    float64
	Notes    []string
...
func NewPyroxeneCriterion() PyroxeneCriterion {
	return PyroxeneCriterion{
		Mineral:     "pyroxene",
		Channel:     "isotope-14",
		Lower:       54.0,
		Upper:       124.0,
...
func (c PyroxeneCriterion) Validate() error {
...
func (c PyroxeneCriterion) Assess(o PyroxeneObservation) (PyroxeneAssessment, error) {
	if err := c.Validate(); err != nil {
		return PyroxeneAssessment{}, err
	}
	if o.Channel != c.Channel {
		return PyroxeneAssessment{}, fmt.Errorf("channel mismatch")
	}
	...
	if math.IsNaN(o.Value) || math.IsInf(o.Value, 0) {
		return PyroxeneAssessment{}, fmt.Errorf("non-finite reading")
	}
	if o.Value < c.Lower {
	...
	return PyroxeneAssessment{Mineral: c.Mineral, Accepted: accepted, Score: score, Notes: notes}, nil
}

func (c PyroxeneCriterion) Explain(o PyroxeneObservation) string {
	a, err := c.Assess(o)
	if err != nil {
		return err.Error()
	}
