...
type MagnetiteCriterion struct {
	Mineral     string
	Channel     string
	Lower       float64
	Upper       float64
...
type MagnetiteObservation struct {
	Channel    string
	Value      float64
	Drift      float64
	Runs       int
...
type MagnetiteAssessment struct {
	Mineral  string
	Accepted bool
	Score    float64
	Notes    []string
...
func NewMagnetiteCriterion() MagnetiteCriterion {
	return MagnetiteCriterion{
		Mineral:     "magnetite",
		Channel:     "isotope-08",
		Lower:       48.0,
		Upper:       118.0,
...
func (c MagnetiteCriterion) Validate() error {
...
func (c MagnetiteCriterion) Assess(o MagnetiteObservation) (MagnetiteAssessment, error) {
	if err := c.Validate(); err != nil {
		return MagnetiteAssessment{}, err
	}
	if o.Channel != c.Channel {
		return MagnetiteAssessment{}, fmt.Errorf("channel mismatch")
	}
	...
	if math.IsNaN(o.Value) || math.IsInf(o.Value, 0) {
		return MagnetiteAssessment{}, fmt.Errorf("non-finite reading")
	}
	if o.Value < c.Lower {
	...
	return MagnetiteAssessment{Mineral: c.Mineral, Accepted: accepted, Score: score, Notes: notes}, nil
}

func (c MagnetiteCriterion) Explain(o MagnetiteObservation) string {
	a, err := c.Assess(o)
	if err != nil {
		return err.Error()
	}
