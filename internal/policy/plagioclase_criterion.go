...
type PlagioclaseCriterion struct {
	Mineral     string
	Channel     string
	Lower       float64
	Upper       float64
...
type PlagioclaseObservation struct {
	Channel    string
	Value      float64
	Drift      float64
	Runs       int
...
type PlagioclaseAssessment struct {
	Mineral  string
	Accepted bool
	Score    float64
	Notes    []string
...
func NewPlagioclaseCriterion() PlagioclaseCriterion {
	return PlagioclaseCriterion{
		Mineral:     "plagioclase",
		Channel:     "isotope-04",
		Lower:       44.0,
		Upper:       114.0,
...
func (c PlagioclaseCriterion) Validate() error {
...
func (c PlagioclaseCriterion) Assess(o PlagioclaseObservation) (PlagioclaseAssessment, error) {
	if err := c.Validate(); err != nil {
		return PlagioclaseAssessment{}, err
	}
	if o.Channel != c.Channel {
		return PlagioclaseAssessment{}, fmt.Errorf("channel mismatch")
	}
	...
	if math.IsNaN(o.Value) || math.IsInf(o.Value, 0) {
		return PlagioclaseAssessment{}, fmt.Errorf("non-finite reading")
	}
	if o.Value < c.Lower {
	...
	return PlagioclaseAssessment{Mineral: c.Mineral, Accepted: accepted, Score: score, Notes: notes}, nil
}

func (c PlagioclaseCriterion) Explain(o PlagioclaseObservation) string {
	a, err := c.Assess(o)
	if err != nil {
		return err.Error()
	}
